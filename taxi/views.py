...
def remove_assign_driver_view(request, car_id) -> HttpResponseRedirect:
    if Car.objects.get(id=car_id) in request.user.cars.all():
        request.user.cars.remove(car_id)
    else:
        request.user.cars.add(car_id)
